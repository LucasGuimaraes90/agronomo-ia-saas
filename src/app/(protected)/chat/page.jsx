...
export default function ChatPage() {
  ...
  async function gerarArquivo(msgs, formato) {
    // 1. IA gera o conteúdo (servidor, rápido)
    ...
    const { titulo, conteudo } = await res.json();

    // 2. Browser gera o arquivo (sem servidor, sem timeout)
    const { generateDocx, generatePptx, generateXlsx } = await import('@/lib/docGenerator');
    let blob;
    if (formato === 'docx') blob = await generateDocx(titulo, conteudo);
    else if (formato === 'pptx') blob = await generatePptx(titulo, conteudo);
    else if (formato === 'xlsx') blob = await generateXlsx(titulo, conteudo);

    const nomeArquivo = titulo.replace(/[^a-zA-Z0-9À-ÿ\s]/g, '').trim().substring(0, 50) || 'agronomo_ia';
    downloadBlob(blob, `${nomeArquivo}.${formato}`);
  }

  async function exportarComMsgs(msgs, formato) {
    setGenerating(formato);
    try {
      await gerarArquivo(msgs, formato);
    } catch (err) {
  ...
  async function exportar(formato) {
    ...
    try {
      ...
      if (formato === 'docx' || formato === 'pptx' || formato === 'xlsx') {
        await gerarArquivo(msgsFiltradas, formato);
      } else {
